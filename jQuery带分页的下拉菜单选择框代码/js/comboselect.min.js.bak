; (function($) {
    ...
    $.extend(ComboSelect.prototype, {
        ...
        _setElem: function(combo_input, option) {
            var elem = {};
            elem.combo_input = $(combo_input).attr({
                'autocomplete': 'off'
        ...
        _afterInit: function(self, data) {
            ...
            if (self.option.multiple) {
                ...
                function(i, row) {
                    var item = {
                        text: row[self.option.field],
                        value: row[self.option.primary_key]
                    };
        ...
        _ehComboInput: function() {
            ...
            if (self.option.multiple) {
                ...
                $(self.elem.element_box).on('click', 'span.tag_close',
                function() {
        ...
        _ehWhole: function() {
            ...
            $(document).off('mousedown.comboSelect').on('mousedown.comboSelect',
            function(e) {
        ...
        _ehNaviPaging: function(self) {
            $('li.csFirstPage', $(self.elem.navi)).off('click').on('click',
            function(ev) {
                $(self.elem.combo_input).focus();
                ev.preventDefault();
                self._firstPage(self);
            });
            $('li.csPreviousPage', $(self.elem.navi)).off('click').on('click',
            function(ev) {
                $(self.elem.combo_input).focus();
                ev.preventDefault();
                self._prevPage(self);
            });
            $('li.csNextPage', $(self.elem.navi)).off('click').on('click',
            function(ev) {
                $(self.elem.combo_input).focus();
                ev.preventDefault();
                self._nextPage(self);
            });
            $('li.csLastPage', $(self.elem.navi)).off('click').on('click',
            function(ev) {
                $(self.elem.combo_input).focus();
                ev.preventDefault();
                self._lastPage(self);
        ...
        _checkValue: function(self) {
            ...
            if (now_value != self.prop.prev_value) {
                ...
                self.prop.is_suggest = true; //是否查后台
                self._suggest(self);

				//可以做些特殊处理，比如每清一个字符重新查，没有关键字则不查
				/*self.prop.is_suggest = true; //是否查后台
				if(now_value=="" || now_value==undefined){
                	$(self.elem.navi_p).hide();
                    $(self.elem.results).empty();
                    self._calcWidthResults(self);
                    self._hideResults(self); //隐藏匹配列表和分页
                }else{
                	self.prop.page_all=1; //重新检索后当前页置为1
                	self._suggest(self); //展示匹配列表
                }
				*/
        ...
        _suggest: function(self) {
            ...
            else self._searchForDb(self, q_word, which_page_num); //请求后台
        },
        ...
        _searchForDb: function(self, q_word, which_page_num) {
            ...
            var _orgParams = {
                q_word: q_word,    //关键字，这个字段名可以option中设置
                pageNumber: which_page_num, //当前页
                pageSize: self.option.per_page, //每页显示条数
            ...
            self.prop.xhr = $.ajax({
                ...
                success: function(returnData) {
                    ...
                    var json = {};
                    json.originalResult = returnData.gridResult.list; //返回的数据列表，这里跟据后台返回数据进行调整
                    json.cnt_whole = returnData.gridResult.totalRow; //记录总行数
                    json.candidate = [];
                    json.primary_key = [];
        ...
        _setNavi: function(self, cnt_whole, cnt_page, page_num) {
            var buildPageNav = function(self, pagebar, page_num, last_page) {
                if ($('li', $(pagebar)).size() == 0) {
                    ...
                    var iconFist = 'fa fa-step-backward',
                    iconPrev = 'fa fa-backward',
                    iconNext = 'fa fa-forward',
                    iconLast = 'fa fa-step-forward';
                    if (!isNewFontAwesome) {
